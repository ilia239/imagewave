...
class SineGenerator:
    ...
    def _generate_varying_line_wave(self, column_intensities, width, base_y, line_idx=None):
        ...
        # Create one coordinate pair per pixel column
        x_coords = np.arange(width, dtype=float)
        y_coords = np.zeros(width)
        
        for col_idx, intensity in enumerate(column_intensities):
            frequency = self.frequency_mapper.get_frequency(intensity)
            
            # Calculate sine value for this column position
            # Use column index as x position for wave calculation
            wave_frequency = frequency * 2 * np.pi / width
            y_coords[col_idx] = base_y + self.amplitude * np.sin(wave_frequency * col_idx)
            
            if line_idx is not None and line_idx < 3 and col_idx < 5:
                logger.debug(f"Line {line_idx}, Col {col_idx}: intensity={intensity:.2f} -> freq={frequency:.2f} -> y={y_coords[col_idx]:.2f}")
